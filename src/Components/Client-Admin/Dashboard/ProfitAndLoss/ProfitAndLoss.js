...
const ProfitAndLoss = ({ widgetData, marketPlaceId, brand_id, product_id, manufacturer_name, fulfillment_channel, DateStartDate, DateEndDate }) => {
  ...
  const handleDownloadOptionClick = async (option) => {
    ...
    if (option === 'PDF' && chartContainerRef.current) {
      ...
      const csvData = [
        ...
        `Channel Fees,<span class="math-inline">\{summaryOther?\.current?\.channel_fee?\.toFixed\(2\)\},</span>{summaryOther?.previous?.channel_fee?.toFixed(2)},${(summaryOther?.current?.channel_fee - summaryOther?.previous?.channel_fee)?.toFixed(2)}`,
        ...
        `Total Tax (Cost),<span class="math-inline">\{summaryOther?\.current?\.totalTaxCost?\.toFixed\(2\)\},</span>{summaryOther?.previous?.totalTaxCost?.toFixed(2)},${(summaryOther?.current?.totalCosts - summaryOther?.previous?.totalCosts)?.toFixed(2)}`,
...
const formattedDateRange = fromDate && toDate
  ...
  const summaryData = [
    ...
    { label: "Channel Fees", value: `$${summaryOther?.current?.channel_fee?.toFixed(2) ?? '0.00'}`, delta: (summaryOther?.current?.channel_fee - summaryOther?.previous?.channel_fee)?.toFixed(2) },
          ...
          <div style={{ width: "65%", height: '450px' }} ref={chartContainerRef}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={processedChartData} margin={{ top: 10, right: hasUnits ? 50 : 30, left: 0, bottom: 30 }}>
                ...
                {graph?.find(g => g.metric === 'grossRevenue') && <Line type="monotone" dataKey="grossRevenue" stroke="#6F42C1" name="Gross Revenue"   strokeWidth={2} dot={false} activeDot={{ r: 5 }} yAxisId="left" />}
                ...
                {graph?.find(g => g.metric === 'expenses') && <Line type="monotone" dataKey="expenses" stroke="#DC3545" name="Expenses"   strokeWidth={2} dot={false} activeDot={{ r: 5 }} yAxisId="left" />}
                {graph?.find(g => g.metric === 'netProfit') && <Line type="monotone" dataKey="netProfit" stroke="#198754" name="Net Profit"   strokeWidth={2} dot={false} activeDot={{ r: 5 }} yAxisId="left" />}
                {graph?.find(g => g.metric === 'units') && <Line type="monotone" dataKey="units" stroke="#b8b8ff" name="Units Sold"   strokeWidth={2} dot={false} activeDot={{ r: 5 }} yAxisId="right" />}
