...
const CustomTooltip = ({ active, payload, label, productList, tab, hoveredProductId }) => {
  ...
  // Filter payload to only include the hovered product
  const filteredPayload = hoveredProductId
    ? payload.filter((entry) => entry.dataKey === hoveredProductId)
    : [];

  // If no product is specifically hovered, or the hovered product is not in the current payload,
...
<NoteModel open={openNote} onClose={() => setOpenNote(false)} />
  ...
  <ResponsiveContainer width="100%" height={400}>
    ...
    >
      {/* Grid lines */}
      <CartesianGrid stroke="#e0e0e0" strokeDasharray="3 3" vertical={false} /> {/* No vertical line on left */}
<XAxis
  dataKey="date"
  ...
  tickFormatter={(val) => {
    ...
    return isTodayOrYesterday ? date.format("h:mm A") : date.format("MMM D");
  }}
/>
