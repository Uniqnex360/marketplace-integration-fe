...
import {
  ...
  Tooltip,
  IconButton,
  useTheme, Popover, Dialog, DialogTitle, DialogContent, DialogActions, Button
} from '@mui/material';
import './Helium.css';
...
const MetricItem = ({
  ...
  return (
    ...
    <CardContent sx={{ py: 0.5 }}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mt={0.5} sx={{paddingRight:'4px'}}>
      ...
      const METRICS_CONFIG = {
        total_orders: {
          ...
          tooltip: (date, today, prev) =>
            date.isSame(today, 'day') ? `Yesterday: ${prev || '0'}` : `${date.subtract(1, 'day').format('MMM DD')}: ${prev || '0'}`,
        },
        total_units: {
          title: 'Units Sold',
          tooltip: (date, today, prev) =>
        ...
        total_cogs: {
          title: 'COGS',
          tooltip: (date, today, prev) =>
            date.isSame(today, 'day') ? `Yesterday: ${formatCurrency(prev)}` : `${date.subtract(1, 'day').format('MMM DD')}: ${formatCurrency(prev)}`,
          currencySymbol: '$',
        },
        net_profit: {
          title: 'Net Profit',
        ...
        margin: {
          title: 'Margin',
          tooltip: (date, today, prev) =>
            date.isSame(today, 'day') ? `Yesterday: ${prev?.toFixed(2)}%` : `${date.subtract(1, 'day').format('MMM DD')}: ${prev?.toFixed(2)}%`,
          percentSymbol: '%',
        },
        business_value: {
  ...
  >
    ...
    <Box sx={{ ...metricBlockStyle, borderRight: '1px solid #e0e0e0', borderLeft: 'none' }}>
      <Box display="flex" alignItems="center" gap={1}>
        ...
        <Tooltip title={selectedDate.format('DD/MM/YYYY')}>
          <Box>
            <Typography fontWeight="bold" sx={{ color: '#485E75', fontFamily: "'Nunito Sans', sans-serif", fontSize: 14 }}>
   ...
   {visibleMetrics.includes('gross_revenue') && (
      ...
      {visibleMetrics.includes('gross_revenue') && (
        <MetricItem
          ...
          tooltip={
            selectedDate.isSame(today, 'day')
              ? `Yesterday: ${formatCurrency(previous.gross_revenue)}`
              : `${selectedDate.subtract(1, 'day').format('MMM DD')}: ${formatCurrency(previous.gross_revenue)}`
          }
          currencySymbol="$"
        />
      )}
    ...
    <Box sx={{ borderRight: '1px solid #e0e0e0' }}>
      ...
      >
        <Box sx={{ width: '100%', height: 80, position: 'relative', overflow: 'visible' }}>
