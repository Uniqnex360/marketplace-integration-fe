...
import {
    ...
    MenuItem,
    Select,
    Tooltip as MuiTooltip, Tab, Tabs,Chip,Badge,Popover
} from '@mui/material';
...
const NumberIndicator = ({ value, change }) => (
    <Box>
        <Typography sx={{ fontSize: '14px', color: '#1A2027', fontWeight: 500, whiteSpace: 'nowrap' }}>
            {value}
        </Typography>
        {change !== undefined && (
            <Box display="flex" alignItems="center" sx={{ color: change >= 0 ? '#1A9E77' : '#E34750', fontSize: '12px' }}>
...
const MyProductList = ({ widgetData, marketPlaceId, brand_id, product_id, manufacturer_name, fulfillment_channel, DateStartDate, DateEndDate }) => {
    const navigate = useNavigate();
    
  const location = useLocation();
const [selectedImageSize, setSelectedImageSize] = useState('Medium');
  const [selectedColumns, setSelectedColumns] = useState([...allColumns, ...additionalInfo]);
...
const controllerRef = useRef(null);
    ...
    const [sortValues, setSortValues] = useState({ sortBy: null, sortByValue: null }); // State for sorting
...
useEffect(() => {
    ...
    const handleToggleAll = () => {
        if (selectAll) {
            ...
            setSelectedColumns([...getColumnSet(activeColumnCategoryTab, tab === 0), ...additionalInfo]);
        }
    ...
    const handleApplyFilter = (newFilterValues = {}, closePopover = true) => {
        ...
        const finalParentAsin = newFilterValues.parentAsin !== undefined
            ? newFilterValues.parentAsin
            : parentAsin;

        const finalSkuAsin = newFilterValues.skuAsin !== undefined
            ? newFilterValues.skuAsin
            : skuAsin;
        ...
        console.log('Applied Filters:', { parentAsin: finalParentAsin.trim(), skuAsin: finalSkuAsin.trim() });
    ...
    const handleSearchChange = (e) => {
        ...
        if (e.target.value) { // Only reset page to 1 if there's a search query
            setPage(1);
            setRowsPerPage(10)
        }
    };
    ...
    const getColumnSet = (columnCategoryIndex, isParentTab) => {
        ...
        if (isParentTab) { // If it's the Parent tab
        ...
        if (columnCategoryIndex === 0 || columnCategoryIndex === 3) {
            if (isParentTab) {
                ...
                finalColumns = finalColumns.filter(
                    col => !['ListingScore', 'Fulfilment status']
                        .includes(col)
                );
            } else {
                ...
                finalColumns = finalColumns.filter(col => col !== 'current price Range');
            }
        } else {
            // Other tabs: remove all 3
            finalColumns = finalColumns.filter(
                col => !['ListingScore', 'Fulfilment status', 'current price Range'].includes(col)
            );
        }
...
useEffect(() => {
  ...
  controllerRef.current = new AbortController();
    ...
    return (
        ...
        <Box sx={{ borderRadius: 1, border: '1px solid #ccc', width: '98%', padding: '4px' }}>
            {/* My Products Title and Tabs */}
        
            <Box display="flex" sx={{ borderBottom: '1px solid #ddd', padding: '4px' }} alignItems="center" mb={2}>
                <Typography variant="h6" component="div" sx={{
                    fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",
                    fontWeight: 600, fontSize: '20px', color: '#1A2027'
                }}>
                    My Products
                </Typography>
                <Box
            ...
            <Box display="flex" alignItems="center" justifyContent="space-between">
                <Box display="flex" alignItems="center" gap={1}>
                    <Typography variant="body2" sx={{
                        fontSize: '16px', fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",
                        color: '#485E75'
                    }}>
                ...
                <Box p={2} display="flex" flexDirection="column" gap={1}>
                    ...
                    <Box display="flex" justifyContent="flex-end" gap={1}>
                        ...
                        <Button
                            variant="contained"
                            size="small"
                            onClick={() => handleApplyFilter({ parentAsin, skuAsin }, true)} // Pass true to close popover
                            disabled={!hasChanges}
                            sx={{
                                fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",
                                fontSize: '14px',
      ...
      InputProps={{
        startAdornment: (
          <InputAdornment position="start">
            <IconButton onClick={() => setShowSearch(false)} size="small" sx={{ p: 0.5 }}>
              <SearchIcon sx={{ color: '#485E75', fontSize: '18px' }} />
            </IconButton>
          </InputAdornment>
        ),
      }}
                ...
                <Box display="flex" alignItems="center" gap={0.5}>
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 0 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 0 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 1 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 1 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 2 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 2 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 3 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 3 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 4 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 4 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
                    ...
                    <Button
                        ...
                        sx={{
                            ...
                            backgroundColor: activeColumnCategoryTab === 5 ? '#19232E' : 'transparent',
                            border: activeColumnCategoryTab === 5 ? 'none' : `1px solid #D3D3D3`,
                            borderRadius: '8px',
                        }}
      ...
      <Box display="flex" gap={1} flexWrap="wrap" sx={{ marginTop: '7px' }}>
                {filterParent && (
                    <Chip
                        ...
                        onDelete={() => handleApplyFilter({ parentAsin: '', skuAsin: filterSku }, false)} // Pass false to prevent closing popover
                ...
                {filterSku && (
                    <Chip
                        ...
                        onDelete={() => handleApplyFilter({ parentAsin: filterParent, skuAsin: '' }, false)} // Pass false to prevent closing popover
                        deleteIcon={<CloseIcon sx={{ color: '#fff' }} />}
                        size="small"
                        sx={{
                            fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif",
...
<Box sx={{ minHeight: 200 }}>
  {loading ? (
    // Show the loading skeleton centered vertically
    <Box
      sx={{
        display: 'flex',
        justifyContent: 'center',
        alignItems: 'center',
        minHeight: 300, // Ensures space is reserved
        width: '100%',
      }}
    >
      <SkeletonTableMyProduct />
    </Box>
  ) : (
    // Show your product table when loading is false
    <MyProductTable
      products={products}
    ...
    />
            ...
            {/* Pagination */}
            <Box display="flex" alignItems="center" mt={2} sx={{ borderTop: '1px solid #eee', pt: 1 }}>
