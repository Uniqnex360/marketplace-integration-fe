...
>
    ...
    {row.marketplace}
             ...
             <TableCell sx={{ ...cellStyle, borderBottom: 'none', color: row.currency_list[0]?.grossRevenue < 0 ? 'red' : 'black' }}>
    {formatCurrency(row.currency_list[0]?.grossRevenue)}
</TableCell>
<TableCell sx={{ ...cellStyle, borderBottom: 'none', color: row.currency_list[0]?.expenses < 0 ? 'red' : 'black' }}>
    {formatCurrency(row.currency_list[0]?.expenses)}
</TableCell>
<TableCell sx={{ ...cellStyle, borderBottom: 'none', color: row.currency_list[0]?.total_cogs < 0 ? 'red' : 'black' }}>
    {formatCurrency(row.currency_list[0]?.total_cogs)}
</TableCell>
<TableCell sx={{ ...cellStyle, borderBottom: 'none', color: row.currency_list[0]?.netProfit < 0 ? 'red' : 'black' }}>
                    ...
                    <Collapse in={open} timeout="auto" unmountOnExit>
                        <Box sx={{ margin: 1 }}>
                            {/* <Typography variant="h6" gutterBottom component="div">
                                More Details for {row.marketplace}
                            </Typography> */}
...
export default function AllMarketplace({ widgetData, marketPlaceId, brand_id, product_id, manufacturer_name, fulfillment_channel, DateStartDate, DateEndDate }) {
    ...
    const handleDownloadCSV = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_IP}downloadMarketplaceDataCSV/`, {
    ...
    const handleDownloadXLS = async () => {
        try {
            const response = await axios.post(`${process.env.REACT_APP_IP}allMarketplaceDataxl/`, {
                marketplace_id: marketPlaceId.id,
                 preset: widgetData,
                brand_id: brand_id,
                product_id: product_id,
    ...
    const formattedDateRange = fromDate && toDate ? `${dayjs(fromDate).format('MMM DD, YYYY')} - ${dayjs(toDate).format('MMM DD, YYYY')}` : '';
    ...
    return (
        <Paper elevation={3} sx={{ marginTop: '10px', boxShadow: 'none', p: 4, border: '1px solid #e0e0e0', borderRadius: '8px' }}>
            <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                <Box>
                    <Typography variant="h5" sx={{ ...fontStyles, fontWeight: 600, fontSize: '20px', color: '#111827' }}>
                        All Marketplaces
                    </Typography>
                ...
                <Box display="flex" alignItems="center">
                    <Tooltip
                        title={
                            <Typography sx={{ fontWeight: 100, fontSize: '14px', color: '#485E75' }}>
                                All currencies have been converted to <Box component="span" sx={{ fontWeight: 700 }}>$ USD</Box> based on the daily exchange rate
                            </Typography>
                        }
                        open={openTooltip}
                    ...
                    <Menu
                        ...
                        PaperProps={{ style: { width: 200, borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,0.1)' } }}
                    >
                        <MenuItem onClick={() => { handleDownloadCSV(); handleMenuClose(); }} sx={{ color: 'rgb(72, 94, 117)', fontFamily: "'Nunito Sans', sans-serif", fontSize: 14 }}>
                            <ListItemIcon sx={{ color: 'rgb(72, 94, 117)', minWidth: 36 }}>
                                <InsertDriveFileIcon sx={{ color: 'rgb(72, 94, 117)', fontSize: '16px' }} />
                            </ListItemIcon>
                            <ListItemText sx={{ fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif", fontSize: '16px' }} primary="Download CSV" />
                        </MenuItem>
                        <MenuItem onClick={() => { handleDownloadXLS(); handleMenuClose(); }} sx={{ color: 'rgb(72, 94, 117)', fontFamily: "'Nunito Sans', sans-serif", fontSize: 14 }}>
                            <ListItemIcon sx={{ color: 'rgb(72, 94, 117)', minWidth: 36 }}>
                                <Download sx={{ color: 'rgb(72, 94, 117)', fontSize: '16px' }} />
                            </ListItemIcon>
                            <ListItemText sx={{ fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif", fontSize: '16px' }} primary="Download XLS" />
                        </MenuItem>
                        <MenuItem onClick={handleMenuClose} sx={{ color: 'rgb(72, 94, 117)', fontFamily: "'Nunito Sans', sans-serif", fontSize: 14 }}>
                            <ListItemIcon sx={{ color: 'rgb(72, 94, 117)', minWidth: 36 }}>
                                <Delete sx={{ color: 'rgb(72, 94, 117)', fontSize: '16px' }} />
                            </ListItemIcon>
                            <ListItemText sx={{ fontFamily: "'Nunito Sans', -apple-system, 'Segoe UI', 'Roboto', 'Helvetica Neue', 'Arial', sans-serif", fontSize: '16px' }} primary="Remove" />
                        </MenuItem>
                    </Menu>
                </Box>
            </Box>
          { !loading  && (<Grid container spacing={3} mb={2}>
                {[
                 {
  title: 'Gross Revenue',
...
},
{
  ...
  changeType: allMarketplaceData?.netProfit?.delta >= 0 ? 'up' : 'down',
                    ...
                    <Grid item xs={12} sm={6} md={3} key={idx} sx={{ borderLeft: idx !== 0 ? '1px solid #e0e0e0' : 'none', pl: idx !== 0 ? 3 : 0 }}>
                        <Typography sx={{ ...fontStyles, fontWeight: 500, mb: 1 }}>{item.title}</Typography>
                        <Typography variant="h4" sx={{ fontFamily: fontStyles.fontFamily, fontSize: '32px', color: '#111827' }}>{item.value}</Typography>
                        <Typography
                            variant="body2"
                            sx={{
                                fontSize: '16px',
                        ...
                        >
                            {item.change}
                            <Box component="span" sx={{ color: item.changeType === 'down' ? '#dc2626' : '#16a34a', fontSize: '16px' }}>
                                {item.changeType === 'down' ? <ArrowUpwardIcon sx={{ fontSize: '14px', color: 'rgb(51, 204, 153)' }} /> : <ArrowDownwardIcon sx={{ fontSize: '14px', color: 'red' }} />}
                            </Box>
                        </Typography>
                    </Grid>
                ))}
            ...
            <Box display="flex" alignItems="center" mb={2} sx={{ cursor: 'pointer' }} onClick={() => setShowBreakdown(!showBreakdown)}>
                <IconButton size="small" sx={{ color: 'rgb(10, 111, 232)', '&:hover': { color: 'rgb(2, 83, 182)' } }}>
...
>
    <TableRow>
        ...
        <TableCell sx={{ ...fontStyles, fontSize: '12px', color: '#485E75',borderTop:'none' }}>Marketplace</TableCell>
