...
return Object.values(chartData).map((item) => ({
  ...
  compareRefundQuantity: item.compare_refund_quantity ?? null,
   ...
   const CustomTooltip = ({ active, payload, label }) => { // Removed compareFinalDate and compareWiseData from here
    ...
    const formatValue = (val, key) => {
      if (typeof val === 'number') {
        if (key.toLowerCase().includes("margin") || key.toLowerCase() === "acos" || key.toLowerCase() === "tacos") {
          return `${val.toFixed(2)}%`;
        } else if (key.toLowerCase().includes("revenue") || key.toLowerCase().includes("profit") || key.toLowerCase().includes("amount") || key.toLowerCase().includes("spend")) {
          return `$${val.toFixed(2)}`;
    ...
    payload.forEach(item => {
      ...
      const key = isCompare ? item.dataKey.replace("compare", "") : item.dataKey;
...
<YAxis
  ...
  axisLine={false}
  tickFormatter={(value) => {
    const firstCurrency = metrics.find(m => visibleMetrics.includes(m.id) && m.isCurrency);
    const firstNonCurrency = metrics.find(m => visibleMetrics.includes(m.id) && !m.isCurrency && !m.isPercentage);
    if (firstCurrency) return `$${Number(value).toLocaleString()}`;
...
<YAxis
  ...
  hide={!visibleMetrics.some(m =>
    [
      ...
      'compare_orders', 'compare_units_sold', 'compare_refund_amount', 'compare_refund_quantity'
    ].includes(m)
  )}
  style={{
    fontSize: '12px',
...
/>
  ...
  {visibleMetrics.includes('gross_revenue') && (<Line type="monotone"   dataKey="grossRevenue"      dot={false}
            stroke={metricColors.gross_revenue}
            strokeWidth={2}
            name={metricLabels.gross_revenue} yAxisId="left" />
  )}

{visibleMetrics.includes('gross_revenue') && (<Line type="monotone" dataKey="compareGrossRevenue"      dot={false} stroke={metricColors.gross_revenue}
            strokeWidth={2}
             name={`Compare ${metricLabels.gross_revenue}`}  strokeDasharray="5 5" yAxisId="left" />
)}
  {visibleMetrics.includes('profit_margin') && (<Line type="monotone" dataKey="profitMargin"      dot={false}  stroke={metricColors.profit_margin}
            strokeWidth={2}
            name={metricLabels.profit_margin} yAxisId="right-percentage" />
  )}
    {visibleMetrics.includes('profit_margin') && (  
<Line type="monotone" dataKey="compareProfitMargin"      dot={false}  strokeDasharray="5 5" name={`Compare ${metricLabels.profit_margin}`} yAxisId="right-percentage" />
    )}

    
    {visibleMetrics.includes('net_profit') && (
<Line type="monotone" dataKey="netProfit"      dot={false} stroke={metricColors.net_profit}
            strokeWidth={2}
            name={metricLabels.net_profit}  yAxisId="left" />
    )}
    
    {visibleMetrics.includes('net_profit') && (
<Line type="monotone" dataKey="compareNetProfit"      dot={false}  strokeDasharray="5 5" stroke={metricColors.net_profit}
            strokeWidth={2}  name={`Compare ${metricLabels.net_profit}`}  yAxisId="left" />
    )}


    {visibleMetrics.includes('orders') && (
<Line type="monotone" dataKey="orders"      dot={false}  stroke={metricColors.orders}
            strokeWidth={2}
            name={metricLabels.orders} yAxisId="right-number" />)}

    {visibleMetrics.includes('orders') && (
<Line type="monotone" dataKey="compareOrders"      dot={false}  strokeDasharray="5 5" stroke={metricColors.orders}
            strokeWidth={2}  name={`Compare ${metricLabels.orders}`} yAxisId="right-percentage" />
    )}
            
    {visibleMetrics.includes('units_sold') && (

<Line type="monotone" dataKey="unitsSold"      dot={false} stroke={metricColors.units_sold}
            strokeWidth={2} name={metricLabels.units_sold} yAxisId="right-number" />
    )}
    
    {visibleMetrics.includes('units_sold') && (
<Line type="monotone" dataKey="compareUnitsSold"      dot={false} strokeDasharray="5 5"  stroke={metricColors.units_sold}
            strokeWidth={2}  name={`Compare ${metricLabels.units_sold}`} yAxisId="right-number" />
    )}

    {visibleMetrics.includes('refund_amount') && (
<Line type="monotone" dataKey="refundAmount"      dot={false} stroke={metricColors.refund_amount}
            strokeWidth={2} name={metricLabels.refund_amount}  yAxisId="right-number" />
    )}
    
    {visibleMetrics.includes('refund_amount') && (
<Line type="monotone" dataKey="compareRefundAmount"      dot={false} strokeDasharray="5 5"  stroke={metricColors.refund_amount}
            strokeWidth={2}  name={`Compare ${metricLabels.refund_amount}`} yAxisId="right-number" />
    )}

    {visibleMetrics.includes('refund_quantity') && (
<Line type="monotone" dataKey="refundQuantity"      dot={false} stroke={metricColors.refund_quantity}
            strokeWidth={2} name={metricLabels.refund_quantity}  yAxisId="right-number" />
    )}
    
    {visibleMetrics.includes('refund_quantity') && (
<Line type="monotone" dataKey="compareRefundQuantity"      dot={false}  strokeDasharray="5 5" stroke={metricColors.refund_quantity}
            strokeWidth={2}  name={`Compare ${metricLabels.refund_quantity}`} yAxisId="right-number" />
    )}
