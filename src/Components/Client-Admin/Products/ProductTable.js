...
import { Refresh, UploadFile, Download } from "@mui/icons-material";
...
const ProductTable = () => {
  ...
  const [productsCount, setProductsCount] = useState([]); // Seems redundant with productCount
...
const [categories, setCategories] = useState(['All', 'Category 1', 'Category 2']);
  ...
  const [selectedCategory, setSelectedCategory] = useState(() => {
    ...
    return storedCategory ? JSON.parse(storedCategory) : { id: "all", name: "All Channels" };
  });
  ...
  }, [updatedList, UpdatedBrandId, page, rowsPerPage, sortConfig, selectedCategory, searchQuery]); // Dependency array
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (response.data && response.data.data && Array.isArray(response.data.data.product_list)) {
        const products = response.data.data.product_list.map((product) => ({
          ...
          marketplacelogo: product.marketplace_image_url || { image_url: soon, name: "N/A" },
...
const handleResetChange = () => {
  ...
  return (
    <Box sx={{ display: "flex", width: "100%", marginTop: "40px", flexDirection: "column" }}>
    <Box sx={{ color: "#000080", flex: 1, overflow: "hidden", marginTop: "50px" }}>
    
      <Box sx={{ display: "flex", gap: 2, flexDirection: "row" }}>
        {filterVisible && (
          <Box sx={{ width: "215px", display: "flex", flexDirection: "column", position: "sticky", top: 0 }}>
            <FiltersUi  categories={categories}    setCategoryFilterList={setCategoryFilterList}    onProductTypeChange={handleBrandChange}   brandFilterList={brandFilterList}  onBrandTypeChange={handleFilterBrand}/>
          </Box>
        )}
...
<MarketplaceOption handleProduct={handleProduct}  handleCategoryList={handleCategoryList} handleBrandList={handleBrandList} clearChannel={selectedCategory} />
</Box>
    ...
    <Download sx={{ color: "white", fontSize: "20px" }} /> 
       ...
       >
         <Refresh sx={{ color: "white", fontSize: "20px" }} />
          ...
          <Typography variant="body2">Total Products: {productCount ? productCount : '0'}</Typography>
        </Box>
  ...
  <Typography variant="subtitle2" fontWeight="bold">
    ...
    <IconButton
        ...
        <TableCell sx={{ textAlign: "center",   minWidth: 120, backgroundColor: "#f6f6f6" }}>
          <Typography variant="subtitle2" fontWeight="bold">Category
          <IconButton
    ...
    minWidth: 90, // The default minWidth for other columns
          ...
          <Typography variant="subtitle2" fontWeight="bold">Quantity
          <IconButton
        ...
        <TableCell sx={{ textAlign: "center",    minWidth: 70, backgroundColor: "#f6f6f6" }}>
          <Typography variant="subtitle2" fontWeight="bold">Price
          <IconButton
      ...
      {loading && productData.length === 0 ? (
        ...
        productData.map((product) => (
          <TableRow key={product.productId} hover>
            ...
            <Link to={`/Home/products/details/${product.productId}?page=${page}&&rowsPerPage=${rowsPerPage}`} style={{ textDecoration: "none" }}>
            <img
                  src={product.image}
                  alt="Product"
...
>
     <Link to={`/Home/products/details/${product.productId}?page=${page}&&rowsPerPage=${rowsPerPage}`} style={{ color:'#121212', textDecoration: "none" }}>
        {product.sku || "N/A"}
        </Link>
</TableCell>

<TableCell sx={{ textAlign: "center" }}>
            <Link to={`/Home/products/details/${product.productId}?page=${page}&&rowsPerPage=${rowsPerPage}`} style={{ textDecoration: "none", color: "black" }}>
    ...
    <Menu
            ...
            >
              ...
              {currentColumn === "product_title" && (
                <>
                  <MenuItem
                    onClick={() => handleSelectSort("product_title", "asc")}
                  >
                    Sort A-Z
                  </MenuItem>
                  <MenuItem
                    onClick={() => handleSelectSort("product_title", "desc")}
                  >
                    Sort Z-A
                  </MenuItem>
                </>
              ...
              {currentColumn === "quantity" && (
                <>
                  <MenuItem
                    onClick={() => handleSelectSort("quantity", "asc")}
                  >
                 Sort Low to High
                  </MenuItem>
                  <MenuItem
                    onClick={() => handleSelectSort("quantity", "desc")}
                  >
                   Sort High to Low
                  </MenuItem>
                </>
